...
from populse_db.filter import FilterToQuery, literal_parser
...
def create_test_case(**database_creation_parameters):
    ...
    class TestDatabaseMethods(unittest.TestCase):
        ...
        def setUp(self):
            ...
            Creates a temporary folder containing the database file that will
            be used for the test
            """
            self.database_creation_parameters = dict(
                database_creation_parameters
            )
            if "database_url" not in self.database_creation_parameters:
                ...
                self.database_creation_parameters["database_url"] = (
                    "sqlite:///" + path
                )
            else:
                self.temp_folder = None
            self.database_url = self.database_creation_parameters[
                "database_url"
            ]
        ...
        def create_database(self, clear=True):
            ...
            try:
                ...
                if self.database_creation_parameters[
                    "database_url"
        ...
        def test_wrong_constructor_parameters(self):
            ...
            self.assertRaises(
                ValueError, lambda: Database("engine").__enter__()
            )
        ...
        def test_add_field(self):
            ...
            with database as session:
                ...
                session.add_field(
                    "collection1", "BandWidth", FIELD_TYPE_FLOAT, None
                )
                session.add_field(
                ...
                session.add_field(
                    "collection1", "Boolean", FIELD_TYPE_BOOLEAN, None
                )
                session.add_field(
                    "collection1",
                    "Boolean list",
                ...
                self.assertEqual(
                    session.get_field(
                        "collection1", "Bitspervoxel"
                    ).description,
                    "without space",
                )
                self.assertEqual(
                    session.get_field(
                        "collection1", "Bits per voxel"
                    ).description,
                    "with space",
                )
                self.assertEqual(
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_field(
                        True, "Field", FIELD_TYPE_LIST_INTEGER, None
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.add_field(
                        "collection1", None, FIELD_TYPE_LIST_INTEGER, None
                    ),
                )
                self.assertRaises(
                    ValueError,
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_field(
                        "collection1", "Patient Name", FIELD_TYPE_STRING, 1.5
                    ),
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_field(
                        "collection1", "name", FIELD_TYPE_STRING, None
                    ),
                )
        ...
        def test_add_fields(self):
            ...
            with database as session:
                ...
                fields = []
                fields.append(
                    ["collection1", "First name", FIELD_TYPE_STRING, ""]
                )
                fields.append(
                    ["collection1", "Last name", FIELD_TYPE_STRING, ""]
                )
                ...
                self.assertRaises(
                    ValueError, lambda: session.add_fields(fields)
                )
                fields = []
                fields.append("Field")
                self.assertRaises(
        ...
        def test_remove_field(self):
            ...
            with database as session:
                ...
                session.add_field(
                    "current", "SequenceName", FIELD_TYPE_STRING, None
                )
                session.add_field(
                    "current",
                    "Dataset dimensions",
                ...
                # Adding values
                session.add_value(
                    "current", "document1", "PatientName", "Guerbet"
                )
                session.add_value(
                    "current", "document1", "SequenceName", "RARE"
                )
                session.add_value(
                    "current", "document1", "Dataset dimensions", [1, 2]
                )
                ...
                # Testing with list of fields
                session.remove_field(
                    "current", ["SequenceName", "PatientName"]
                )
                self.assertIsNone(session.get_field("current", "SequenceName"))
                self.assertIsNone(session.get_field("current", "PatientName"))
                ...
                self.assertRaises(
                    ...
                    lambda: session.remove_field(
                        "current",
                        ["SequenceName", "PatientName", "Not_Existing"],
                    ),
                )
                ...
                session.add_field(
                    "current", "list1", FIELD_TYPE_LIST_INTEGER, None
                )
                session.add_field(
                    "current", "list2", FIELD_TYPE_LIST_STRING, None
                )
        ...
        def test_get_field(self):
            ...
            with database as session:
                ...
                self.assertIsNotNone(
                    session.get_field("collection1", "PatientName")
                )

                # Testing that None is returned if the field does not exist
                self.assertIsNone(session.get_field("collection1", "Test"))

                # Testing that None is returned if the collection does not
                # exist
                self.assertIsNone(
                    session.get_field("collection_not_existing", "PatientName")
                )

                # Testing that None is returned if both collection and field
                # do not exist
                self.assertIsNone(
                    session.get_field("collection_not_existing", "Test")
                )
        ...
        def test_get_fields(self):
            ...
            with database as session:
                ...
                self.assertEqual(
                    session.get_fields("collection_not_existing"), []
                )
        ...
        def test_set_value(self):
            ...
            with database as session:
                ...
                session.add_value(
                    "collection1", "document1", "PatientName", "test", "test"
                )
                session.set_value(
                ...
                session.add_value(
                    "collection1", "document1", "Bits per voxel", 1, 1
                )
                session.set_value(
                ...
                session.add_value(
                    "collection1", "document1", "AcquisitionDate", date, date
                )
                self.assertEqual(
                    session.get_value(
                        "collection1", "document1", "AcquisitionDate"
                    ),
                    date,
                )
                ...
                session.add_value(
                    "collection1", "document1", "AcquisitionTime", time, time
                )
                self.assertEqual(
                    session.get_value(
                        "collection1", "document1", "AcquisitionTime"
                    ),
                    time,
                )
                ...
                self.assertEqual(
                    session.get_value(
                        "collection1", "document1", "PatientName"
                    ),
                    "test2",
                )
                self.assertEqual(
                ...
                session.set_value(
                    "collection1", "document1", "PatientName", None
                )
                self.assertIsNone(
                    session.get_value(
                        "collection1", "document1", "PatientName"
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document3", "PatientName", None
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document1", "NotExisting", None
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document3", "NotExisting", None
                    ),
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document1", "Bits per voxel", "test"
                    ),
                )
                self.assertEqual(
                    session.get_value(
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document1", "Bits per voxel", 35.8
                    ),
                )
                self.assertEqual(
                    session.get_value(
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        False, "document1", "Bits per voxel", 35
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", 1, "Bits per voxel", "2"
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document1", None, "1"
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value("collection1", 1, None, True),
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.set_value(
                        "collection1", "document1", "name", None
                    ),
                )
        ...
        def test_set_values(self):
            ...
            with database as session:
                ...
                # Adding fields
                session.add_field(
                    "collection1", "SequenceName", FIELD_TYPE_STRING
                )
                session.add_field(
                    "collection1", "PatientName", FIELD_TYPE_STRING
                )
                ...
                # Adding values
                session.add_value(
                    "collection1", "document1", "SequenceName", "Flash"
                )
                session.add_value(
                    "collection1", "document1", "PatientName", "Guerbet"
                )
                session.add_value(
                    "collection1", "document1", "BandWidth", 50000
                )
                self.assertEqual(
                    session.get_value(
                        "collection1", "document1", "SequenceName"
                    ),
                    "Flash",
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    session.get_value("collection1", "document1", "BandWidth"),
                    50000,
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.set_values(
                        "collection1", "document_not_existing", values
                    ),
                )

                # Testing with list values
                session.add_field(
                    "collection1", "list1", FIELD_TYPE_LIST_STRING
                )
                session.add_field(
                    "collection1", "list2", FIELD_TYPE_LIST_INTEGER
                )
                session.add_value(
                    "collection1", "document1", "list1", ["a", "b", "c"]
                )
                session.add_value(
                    "collection1", "document1", "list2", [1, 2, 3]
                )
                values = {}
                values["list1"] = ["a", "a", "a"]
                values["list2"] = [1, 1, 1]
                session.set_values("collection1", "document1", values)
                self.assertEqual(
                    session.get_value("collection1", "document1", "list1"),
                    ["a", "a", "a"],
                )
                self.assertEqual(
                    session.get_value("collection1", "document1", "list2"),
                    [1, 1, 1],
                )
        ...
        def test_get_field_names(self):
            ...
            with database as session:
                ...
                # Testing with a collection not existing
                self.assertEqual(
                    session.get_fields_names("collection_not_existing"), []
                )

        def test_get_value(self):
            """
            Tests the method giving the current value, given a document and
            a field
            """
            ...
            with database as session:
                ...
                session.add_field(
                    "collection1", "Grids spacing", FIELD_TYPE_LIST_FLOAT, None
                )
                ...
                self.assertEqual(
                    session.get_value(
                        "collection1", "document1", "Grids spacing"
                    ),
                    [0.234375, 0.234375, 0.4],
                )
                ...
                self.assertIsNone(
                    session.get_value(
                        "collection_not_existing", "document1", "PatientName"
                    )
                )
                self.assertIsNone(
                    session.get_value(
                        "collection1", "document3", "PatientName"
                    )
                )
                self.assertIsNone(
                    session.get_value(
                        "collection1", "document1", "NotExisting"
                    )
                )
                self.assertIsNone(
                    session.get_value(
                        "collection1", "document3", "NotExisting"
                    )
                )
                self.assertIsNone(
                    session.get_value(
                        "collection1", "document2", "Grids spacing"
                    )
                )

                # Testing with wrong parameters
                self.assertIsNone(
                    session.get_value(3, "document1", "Grids spacing")
                )
                self.assertIsNone(
                    session.get_value("collection1", 1, "Grids spacing")
                )
                self.assertIsNone(
                    session.get_value("collection1", "document1", None)
                )
        ...
        def test_check_type_value(self):
            ...
            # with database as session:
            self.assertTrue(
                DatabaseSession.check_value_type("string", FIELD_TYPE_STRING)
            )
            self.assertFalse(
                DatabaseSession.check_value_type(1, FIELD_TYPE_STRING)
            )
            self.assertTrue(
                DatabaseSession.check_value_type(None, FIELD_TYPE_STRING)
            )
            self.assertTrue(
                DatabaseSession.check_value_type(1, FIELD_TYPE_INTEGER)
            )
            self.assertTrue(
                DatabaseSession.check_value_type(1, FIELD_TYPE_FLOAT)
            )
            self.assertTrue(
                DatabaseSession.check_value_type(1.5, FIELD_TYPE_FLOAT)
            )
            self.assertFalse(DatabaseSession.check_value_type(None, None))
            self.assertTrue(
                DatabaseSession.check_value_type([1.5], FIELD_TYPE_LIST_FLOAT)
            )
            self.assertFalse(
                DatabaseSession.check_value_type(1.5, FIELD_TYPE_LIST_FLOAT)
            )
            self.assertFalse(
                DatabaseSession.check_value_type(
            ...
            value["test2"] = 2
            self.assertTrue(
                DatabaseSession.check_value_type(value, FIELD_TYPE_JSON)
            )
            value2 = {}
            value2["test3"] = 1
            value2["test4"] = 2
            self.assertTrue(
                DatabaseSession.check_value_type(
                    [value, value2], FIELD_TYPE_LIST_JSON
                )
            )
        ...
        def test_add_value(self):
            ...
            with database as session:
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", "document3", "SequenceName", "none"
                    ),
                )
                self.assertRaises(
                    ValueError,
                ...
                self.assertIsNone(
                    session.add_value(
                        "collection1", "document1", "BandWidth", 45
                    )
                )
                ...
                self.assertEqual(
                    session.get_value("collection1", "document2", "BandWidth"),
                    35.5,
                )
                self.assertEqual(
                    session.get_value(
                ...
                self.assertEqual(
                    session.get_value("collection1", "document1", "Boolean"),
                    True,
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", "document2", "Bits per voxel", 35.5
                    ),
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", "document1", "BandWidth", "test", "test"
                    ),
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        5, "document1", "Grids spacing", "2", "2"
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", 1, "Grids spacing", "2", "2"
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", "document1", None, "1", "1"
                    ),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", "document1", "PatientName", None, None
                    ),
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value("collection1", 1, None, True),
                )
                self.assertRaises(
                    ValueError,
                    lambda: session.add_value(
                        "collection1", "document2", "Boolean", "boolean"
                    ),
                )
        ...
        def test_get_document(self):
            ...
            with database as session:
                ...
                self.assertIsNotNone(
                    session.get_document("collection1", "document1")
                )

                # Testing that None is returned if the document does not exist
                self.assertIsNone(
                    session.get_document("collection1", "document3")
                )

                # Testing that None is returned if the collection does
                # not exist
                self.assertIsNone(
        ...
        def test_remove_document(self):
            ...
            with database as session:
                ...
                session.add_field(
                    "collection1", "FOV", FIELD_TYPE_LIST_INTEGER, None
                )
                ...
                self.assertIsNone(
                    session.get_document("collection1", "document2")
                )
        ...
        def test_add_document(self):
            ...
            with database as session:
                ...
                # Adding fields
                session.add_field(
                    "collection1", "List", FIELD_TYPE_LIST_INTEGER
                )
                ...
                self.assertRaises(
                    ValueError,
                    lambda: session.add_document("collection1", True),
                )
                ...
                # Adding a document with missing field, without the option to
                # add missing fields
                with self.assertRaises(ValueError):
                    document = {}
        ...
        def test_add_collection(self):
            ...
            with database as session:
                ...
                self.assertRaises(
                    ValueError, lambda: session.add_collection("collection1")
                )

                # Trying with table names already taken
                self.assertRaises(
                    ValueError, lambda: session.add_collection("_field")
                )

                self.assertRaises(
                    ValueError, lambda: session.add_collection("_collection")
                )

                # Trying with wrong types
                self.assertRaises(
                    ValueError, lambda: session.add_collection(True)
                )
                self.assertRaises(
                    ValueError,
        ...
        def test_remove_collection(self):
            ...
            with database as session:
                ...
                # Removing one collection and testing that the other is
                # unchanged
                ...
                # Removing the collection containing the field and the
                # document and testing that it is indeed removed
                session.remove_collection("collection1")
                self.assertIsNone(session.get_collection("collection1"))
                self.assertIsNone(session.get_field("collection1", "name"))
                self.assertIsNone(session.get_field("collection1", "Field"))
                self.assertIsNone(
                    session.get_document("collection1", "document")
                )

                # Testing with a collection not existing
                self.assertRaises(
                    ValueError,
                    lambda: session.remove_collection(
                        "collection_not_existing"
                    ),
                )
                self.assertRaises(
                    ValueError, lambda: session.remove_collection(True)
                )
        ...
        def test_get_collections_names(self):
            ...
            with database as session:
                ...
                self.assertEqual(
                    session.get_collections_names(), ["collection1"]
                )
        ...
        def test_get_documents(self):
            """
            Tests the method returning the list of document rows, given a
            collection
            """
            ...
            with database as session:
                ...
                # Testing with a collection not existing
                self.assertEqual(
                    session.get_documents("collection_not_existing"), []
                )
        ...
        def test_get_documents_names(self):
            """
            Tests the method returning the list of document names, given a
            collection
            """
            ...
            with database as session:
                ...
                # Testing with a collection not existing
                self.assertEqual(
                    session.get_documents_names("collection_not_existing"), []
                )
        ...
        def test_list_dates(self):
            """
            Tests the storage and retrieval of fields of type list of time,
            date and datetime
            """
            ...
            with database as session:
                ...
                # Adding fields
                session.add_field(
                    "collection1", "list_date", FIELD_TYPE_LIST_DATE, None
                )
                session.add_field(
                    "collection1", "list_time", FIELD_TYPE_LIST_TIME, None
                )
                session.add_field(
                    "collection1",
                    "list_datetime",
                ...
                list_time = [
                    datetime.time(12, 41, 33, 540),
                    datetime.time(1, 2, 3),
                ]
                list_datetime = [
                    datetime.datetime(2018, 5, 23, 12, 41, 33, 540),
                    datetime.datetime(1899, 12, 31, 1, 2, 3),
                ]

                session.add_value(
                    "collection1", "document1", "list_date", list_date
                )
                self.assertEqual(
                    list_date,
                    session.get_value("collection1", "document1", "list_date"),
                )
                session.add_value(
                    "collection1", "document1", "list_time", list_time
                )
                self.assertEqual(
                    list_time,
                    session.get_value("collection1", "document1", "list_time"),
                )
                session.add_value(
                    "collection1", "document1", "list_datetime", list_datetime
                )
                self.assertEqual(
                    list_datetime,
                    session.get_value(
        ...
        def test_json_field(self):
            ...
            with database as session:
                ...
                self.assertEqual(
                    doc,
                    session.get_document("collection1", "the_name")._dict(),
                )
                self.assertIsNone(
        ...
        def test_filter_documents(self):
            ...
            with database as session:
                ...
                session.add_field(
                    "collection_test", "field_test", FIELD_TYPE_STRING, None
                )
                ...
                # Checking that every document is returned if there is no
                # filter
                documents = set(
                    document.index
                    for document in session.filter_documents(
        ...
        def test_filters(self):
            ...
            with database as session:
                ...
                for filter, expected in (
                    ...
                    (
                        '(format == "NIFTI" OR NOT format == "DICOM") '
                        'AND ("a" IN strings OR NOT "b" IN strings)',
                        set(
                            [
                                "/abc_1899.none",
                                "/xyz_1899.mgz",
        ...
        def test_modify_list_field(self):
            ...
            with database as session:
                ...
                session.add_value(
                    "collection1", "test", "strings", ["a", "b", "c"]
                )
                names = list(
                    document.name
                    for document in session.filter_documents(
                ...
                session.set_value(
                    "collection1", "test", "strings", ["x", "y", "z"]
                )
                names = list(
                    document.name
                    for document in session.filter_documents(
        ...
        def test_with(self):
            ...
            with database as session:
                names = [
                    i.name
                    for i in session.filter_documents("collection1", "all")
                ]
